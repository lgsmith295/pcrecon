...
#'
#' @examples
eval_clim <- function(crns, lag = 1, prewhiten.crn = TRUE, climate, mos = 5:8, method = "mean", prewhiten.clim = TRUE, calib, valid, cor.window = "calib", type = "pearson", alternative = "two.sided", r = 0.25, alpha = 0.90, print.out = TRUE, save.out = "R", dir = "PCregOutput/") {
...
if(!all(full %in% clim$year)) {
  ...
  PCR_crns <- filter_cor(crns = crns, clim = clim, lag = lag, cor.window = cor.window, type = type, alternative = alternative, r = r, alpha = alpha, prewhiten.crn = prewhiten.crn, prewhiten.clim = prewhiten.clim, calib = calib, full = full, valid = valid)
