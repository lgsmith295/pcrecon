...
#' @param lag single value indicating the maximum number of years to shift chronology data forward. 0 indicates no lag, 2 will evaluate 0, 1, and 2 year lags, etc. Defaults to 1.
...
#'
#' @examples
filter_cor <- function(crns, lag = 1, clim, cor.window, type = "pearson", alternative = "two.sided", r = 0.25, alpha = 0.90, prewhiten.crn = TRUE, prewhiten.clim = TRUE, calib = calib, valid = valid, full = full){
...
if(isTRUE(prewhiten.crn)){
  ...
  rows <- ncol(df$crn_window) * length(lags)

  cors_table <- as.data.frame(matrix(NA, nrow = rows, ncol = 4))
  names <-  c("chronology", "lags", "correlation", "p_value")
 ...
 for (j in 1:length(lags)) {
  for (i in 1:length(crn_names)) {
    ...
    cors_table[k, ] <- cbind(crn_names[i], lags[j], cor$estimate, cor$p.value)
}
 }
    class(cors_table$p_value) <- "numeric"
    class(cors_table$correlation) <- "numeric"
    class(cors_table$lags) <- "integer"
...
#' Fill table of chronologies and lag chronologies that pass correlation filtering
#'
#' @param crns
#' @param cors_table_small dataframe of filtered chronology names, lags, correlation, and p values.
#'
#' @return dataframe of chronologies, including lag chronologies, that met threshold correlation values for analysis
#'
#'
#' @examples
#'
crns_table <- function(crns = crns, cors_table_small = cors_table_small) {
 for (i in 1:nrow(cors_table_small)) {
  ...
  if (cors_table_small$lags[i] != 0) {
    lead_val <- as.integer(cors_table_small$lags[i])
    chronology_name <- paste0(cors_table_small$chronology[i], "_lag", lead_val)
    chron_lead <- dplyr::lag(chron, lead_val )
